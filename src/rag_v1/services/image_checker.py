...
def _score_single_doc(
    ...
    if not image_urls:
        ...
        cached_ready = bool(doc.get("clip_page_image_embeddings_ready"))
        ...
        if page_embeddings.size == 0:
            match_score = 0.0
            match_status = "download_failed" if cached_ready and failures else "failed"
            if failures and match_status == "failed":
